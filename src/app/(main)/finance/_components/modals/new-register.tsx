...
export const NewRegister = ({
  ...
  const [type, setType] = useState<
    ETransactionTypeDonation | ETransactionTypeExpense
  ...
  const steps =
    type === "donation" ? mockedSteps.donation : mockedSteps.expensive;

  const formDonationStepTwo = useForm<z.infer<typeof DonationStepTwoSchema>>({
    resolver: zodResolver(DonationStepTwoSchema),
  });

  const formDonationStepThree = useForm<
    z.infer<typeof DonationStepThreeSchema>
  >({
  ...
  const handleOnSubmitStepTwoExpensive = async (
    data: z.infer<typeof ExpensiveStepTwoSchema>
  ) => {
    try {
      const form = {
  ...
  if (type === ETransactionTypeExpense.Expense && step === 2) {
    content = (
      <Form {...formExpensiveStepTwo}>
        <form
          onSubmit={formExpensiveStepTwo.handleSubmit(
            handleOnSubmitStepTwoExpensive
          )}
          className="flex flex-col gap-6"
        >
          <div className="grid gap-2">
